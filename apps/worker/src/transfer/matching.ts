// matching.ts — XFER-1's transfer-matching candidate logic (pure
...
// certainly the two sides of one cross-account transfer. The queue glue
// (../queues/transferMatching.ts) is what loads the pool and writes the
// result -- same split as categorize/tier1.ts vs. sync/syncConnection.ts.
