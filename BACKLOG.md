...
- [ ] **CAT-10** — Category colors. Save a default color into the default categories model (CAT-9), changeable by the user; custom categories a user adds get the same treatment (a color field on the same per-user category record, set on creation and editable afterward).
